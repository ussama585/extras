...
export default function InterviewRoomCompany() {
	...
	const iceServers = [
		...
		{
			urls: 'turn:global.turn.twilio.com:3478?transport=udp',
			username: '03bba1730190920dad2f4f488355b312e89ec0c80c759bb99db222ade0e0a8e4',
			credential: '0iR3hGU9LOVTs+ffBWHi0/7LQMUEJNY95IP8JiuZfEA='
		},
		{
			urls: 'turn:global.turn.twilio.com:3478?transport=tcp',
			username: '03bba1730190920dad2f4f488355b312e89ec0c80c759bb99db222ade0e0a8e4',
	...
	const handleWebSocketMessage = async (parsedMessage) => {
		if (parsedMessage.error) {
			...
			if (userType === "company") {
				navigate(`/company/interview-room/${id}`);
			} else {
				navigate(`/candidate/interview-room/${id}`);
			}
		}
	...
	const { sendJsonMessage } = useWebSocket(
		`wss://04ae12371a71.ngrok.app/ws/video/?token=${accessToken}&id=${id}`,
		{
	...
	const handleCallUser = async () => {
		try {
			...
			console.log("Local Tracks: ", localMediaStream);

			setLocalStream(localMediaStream);
			// Mute local video to avoid duplicate voice
			const localVideoElement = document.querySelector(".local-video");
			if (localVideoElement) {
				localVideoElement.muted = true;  // Ensure the local stream is muted for playback
			}
		...
		if (isScreenShared) {
			...
			try {
				...
				stream.getTracks()[0].onended = () => {
					...
					if (videoSender) {
						videoSender.replaceTrack(videoTrack); // Replace the track back to camera
					}
	...
	useEffect(() => {
		...
		const details = sessionStorage.getItem('interviewDetails');
		if (details) {
			...
			<Card className="video-card">
				...
				<CardBody className="video-card-body">
					...
					<Row className="video-row mt-4">
						{isScreenShared ? (
							<>
								<Col md={8} className="video-column">
									{sharedScreenStream && (
										<div className="video-container shared-video" id="sharedScreenContainer">
											<video
												className="shared-video"
												autoPlay
												playsInline
												ref={(ref) => {
													if (ref && sharedScreenStream) {
														ref.srcObject = sharedScreenStream;
													}
												}}
											/>
											<div className="video-name">Shared Screen</div>
										</div>
									)}
								</Col>
								<Col md={4} className="video-column flex-column">
									{localStream && (
										<div
									...
									<Col md={6} className="video-column">
										...
										>
											<video
												...
												className="remote-video"
												style={{
													// transform: isScreenShared ? "none" : "scale(-1, 1)"
												  }}
												ref={(ref) => {
													if (ref && remoteStream) {
														ref.srcObject = remoteStream;
						...
						{onGoingCall && (
							<div className="button-controls">
								...
								<Button
									...
									className="call-button"
								>
									{isCameraEnabled ? <CameraAltIcon /> : <NoPhotographyIcon />}
								</Button>
								<Button
									color="secondary"
									...
									className="call-button"
								>
									{isMicrophoneEnabled ? <KeyboardVoiceIcon /> : <MicOffIcon />}
								</Button>
								<Button
									color="secondary"
